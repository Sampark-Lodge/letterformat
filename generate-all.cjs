...
// Read the pages.ts file
const pagesFile = fs.readFileSync('./src/content/data/pages.ts', 'utf-8');

// Extract all pages using regex
function extractPages() {
  const pages = [];
  const keywordRegex = /keyword:\s*['"]([^'"]+)['"]/g;
  let match;
  
  while ((match = keywordRegex.exec(pagesFile)) !== null) {
    const startIndex = match.index;
    const keyword = match[1];
    
    // Find the end of this object
    const nextKeyword = pagesFile.indexOf('keyword:', startIndex + 10);
    const endIndex = nextKeyword === -1 ? pagesFile.length : nextKeyword;
    const objectStr = pagesFile.substring(startIndex, endIndex);
    ...
    pages.push({
      ...
      userSegment: extract('userSegment'),
      recipientTitle: extract('recipientTitle'),
      subjectLine: extract('subjectLine'),
      sampleName: extract('sampleName'),
      sampleCity: extract('sampleCity'),
      sampleInstitution: extract('sampleInstitution'),
      sampleDate: extract('sampleDate'),
      signOff: extract('signOff'),
      closingLine: extract('closingLine'),
    });
  }
  return pages;
}
...
// Extract arrays
function extractArrays(pageIndex, pagesFile) {
  const pages = [];
  const keywordRegex = /keyword:\s*['"]([^'"]+)['"]/g;
  let match;
  let count = 0;
  
  while ((match = keywordRegex.exec(pagesFile)) !== null && count < pageIndex + 1) {
    if (count === pageIndex) {
      const startIndex = match.index;
      const nextKeyword = pagesFile.indexOf('keyword:', startIndex + 10);
      const endIndex = nextKeyword === -1 ? pagesFile.length : nextKeyword;
      ...
      // Extract bodyParagraphs
      const bodyMatch = objectStr.match(/bodyParagraphs:\s*\[([\s\S]*?)\]\s*,/);
      const sampleBodyMatch = objectStr.match(/sampleBody:\s*\[([\s\S]*?)\]\s*,/);
      const tipsMatch = objectStr.match(/tips:\s*\[([\s\S]*?)\]\s*,/);
      const mistakesMatch = objectStr.match(/mistakes:\s*\[([\s\S]*?)\]\s*,/);
      const faqsMatch = objectStr.match(/faqs:\s*\[([\s\S]*?)\]\s*relatedLinks/s);
      const relatedMatch = objectStr.match(/relatedLinks:\s*\[([\s\S]*?)\]\s*\}/);
      ...
      const parseFaqs = (m) => {
        ...
        let im;
        while ((im = regex.exec(m[1] || m.input.substring(m.index, m.index + 2000))) !== null) {
          faqs.push({ q: im[1], a: im[2].replace(/\\'/g, "'") });
        }
        return faqs;
      };
      
      const parseRelated = (m) => {
        if (!m) return [];
        const related = [];
        const regex = /\{ text:\s*['"]([^'"]+)['"],\s*slug:\s*['"]([^'"]+)['"] \}/g;
        let im;
        while ((im = regex.exec(m[1])) !== null) {
          related.push({ text: im[1], slug: im[2] });
        }
        return related;
      };
      
      return {
        bodyParagraphs: parseArray(bodyMatch),
        sampleBody: parseArray(sampleBodyMatch),
        tips: parseArray(tipsMatch),
        mistakes: parseArray(mistakesMatch),
        faqs: parseFaqs(faqsMatch),
        relatedLinks: parseRelated(relatedMatch)
      };
    }
    count++;
  }
...
// Category info
const catInfo = {
  application: { label: 'আবেদন', icon: '📝' },
  complaint: { label: 'অভিযোগ', icon: '📢' },
  certificate: { label: 'সার্টিফিকেট', icon: '📜' },
  bank: { label: 'ব্যাংক', icon: '🏦' },
  government: { label: 'সরকারি', icon: '🏛️' },
  school: { label: 'স্কুল', icon: '🏫' },
  employment: { label: 'চাকরি', icon: '💼' }
};

// Create formats directory
const outputDir = './formats';
if (!fs.existsSync(outputDir)) {
  fs.mkdirSync(outputDir);
}

// Generate one page
function generatePage(page, arrays, allPages) {
  const cat = catInfo[page.category] || { label: page.category, icon: '📄' };
  const title = page.keyword.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  
  // Format body
  let formatBody = arrays.bodyParagraphs.map(p => `<p>${p}</p>`).join('\n            ');
  
  // Tips
  let tipsHtml = arrays.tips.map((tip, i) => 
    `<div class="tip-item"><div class="tip-num">${i + 1}</div><div class="tip-text">${tip}</div></div>`
  ).join('\n          ');
  
  // Mistakes
  let mistakesHtml = arrays.mistakes.map((m, i) => 
    `<div class="mistake-item"><div class="mistake-icon">✕</div><div class="mistake-text">${m}</div></div>`
  ).join('\n          ');
  
  // FAQs
  let faqsHtml = arrays.faqs.map(faq => 
    `<div class="faq-item"><h3>❓ ${faq.q}</h3><p>${faq.a}</p></div>`
  ).join('\n          ');
  
  // Related links
  let relatedHtml = arrays.relatedLinks.slice(0, 6).map(link => {
    const linkSlug = link.slug.replace(/^\//, '').replace(/\//g, '-');
    return `<a href="${linkSlug}.html" class="related-card"><span>📄 ${link.text}</span><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>`;
  }).join('\n          ');
  
  const salutation = page.signOff === 'Yours obediently' ? 'মহাশয়/মহাশয়া,' : 
                     page.signOff === 'Yours sincerely' ? 'জনাব,' : 'মহোদয়,';
...
<head>
  ...
  <title>${title} — LetterFormat.in</title>
  <meta name="description" content="বিনামূল্যে ${page.keyword}। কপি-পেস্ট ফরম্যাট, পূরণ করা নমুনা, টিপস সহ। আপডেট ২০২৬।">
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .placeholder{background:rgba(220,38,38,0.08);border-bottom:2px dashed var(--red-mid);padding:0 4px;border-radius:3px;color:var(--red-dark)}
    ...
    @media print{.navbar,.sidebar,.letter-toolbar,.toast,.meta-actions{display:none!important}.letter-wrapper{border:none;box-shadow:none}.body{background:white}}
  </style>
</head>
<body>
<nav class="navbar">
  <div class="container">
    <div class="nav-inner">
      ...
      <div class="breadcrumb">
        ...
        <a href="${page.category}.html">${cat.label}</a>
        <span class="breadcrumb-sep">›</span>
        <span>${page.keyword.split(' ').slice(-2).join(' ')}</span>
      </div>
  ...
  <div class="page-layout">
    <div class="main-content">
      <div class="format-meta">
        <div class="meta-tags">
          <span class="tag red">${cat.icon} ${cat.label}</span>
          <span class="tag green">✅ যাচাইকৃত</span>
        </div>
        <h1 class="format-title">${title}</h1>
        <p class="format-desc">বিনামূল্যে ${page.keyword}। কপি-পেস্ট ফরম্যাট, পূরণ করা নমুনা, টিপস সহ। আপডেট ২০২৬।</p>
        <div class="meta-stats">
          <span>👁 <strong>১২,৪৫৬</strong> বার দেখা হয়েছে</span>
          <span>📥 <strong>৮,২৩৪</strong> বার কপি হয়েছে</span>
          <span>⭐ <strong>৪.৯/৫</strong> রেটিং</span>
        </div>
      ...
      <div class="letter-wrapper">
        <div class="letter-toolbar">
          <div class="toolbar-label"><div class="toolbar-dot"></div>ফরম্যাট — লাল অংশ পরিবর্তন করুন</div>
          <button class="toolbar-copy-btn" onclick="copyLetter()">কপি করুন</button>
        </div>
        <div class="letter-document" id="letterDocument">
          <div class="letter-date">তারিখ: <span class="placeholder">${page.sampleDate || '১ জানুয়ারি, ২০২৬'}</span></div>
          <div class="letter-to">
            <span class="to-label">বরাবর</span>
            <div class="to-name"><span class="placeholder">${page.recipientTitle}</span></div>
            <div class="to-address"><span class="placeholder">${page.sampleInstitution}</span><br><span class="placeholder">${page.sampleCity}</span></div>
          </div>
          <div class="letter-subject">
            <div class="sub-label">বিষয়</div>
            <div class="sub-text"><span class="placeholder">${page.subjectLine}</span></div>
          </div>
          <div class="letter-salutation">${salutation}</div>
          <div class="letter-body">${formatBody}</div>
          <div class="letter-closing">${page.closingLine},</div>
          <div class="sign-section">
            <div class="sign-line"></div>
            <div class="sign-name"><span class="placeholder">${page.sampleName}</span></div>
          </div>
        </div>
      </div>
    ...
    <div class="sidebar">
      ...
      <div class="sidebar-card"><h3>📂 সম্পর্কিত</h3><div class="related-list">${relatedHtml}</div></div>
    </div>
  </div>
</div>
...
pages.forEach((page, i) => {
  const arrays = extractArrays(i, pagesFile);
  if (arrays) {
    const html = generatePage(page, arrays, pages);
...
console.log(`\n✅ Generated ${pages.length} pages in ${outputDir}/`);
